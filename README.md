...
Бот разработан для оптимизации бизнес-процессов сотрудников компании, занимающейся продажей прокси. Он упрощает и ускоряет обработку данных, связанных с прокси и IP-адресами.
...
- [Подробное описание функционала](#подробное-описание-функционала)
  ...
  - [3. Фильтрация IP-адресов - `/filter_by_list`](#3-фильтрация-ip-адресов---filter_by_list)
  - [4. Фильтрация IP-адресов по первому октету - `/filter_by_octet`](#4-фильтрация-ip-адресов-по-первому-октету---filter_by_octet)
  - [5. Обрезать 4-й октет - `/remove_fourth_octet`](#5-обрезать-4-й-октет---remove_fourth_octet)
  - [6. Обрезать порт - `/remove_port`](#6-обрезать-порт---remove_port)
  - [7. Проверка прокси или сайта через пинг - `/ping`](#7-проверка-прокси-или-сайта-через-пинг---ping)
...
- `/filter_by_list` - Отфильтровать IP-адреса по списку.
- `/filter_by_octet` - Отфильтровать IP-адреса по первому октету.
- `/remove_fourth_octet` - Обрезать 4-й октет.
- `/remove_port` - Обрезать порт.
- `/ping` - Проверка прокси или сайта через пинг.
...
- **Кликабельные IP-адреса**: Каждый IP-адрес можно скопировать в буфер обмена нажав на него левой кнопкой мыши или через тап при использовании мобильной версии Telegram.
...
### 3. Фильтрация IP-адресов по списку - /filter_by_list
...
### 4. Фильтрация IP-адресов по первому октету - /filter_by_octet
...
### 5. Обрезать 4-ый октет - /remove_fourth_octet
...
### 7. Проверить прокси или сайт через пинг - /ping

1. Введите IP-адрес прокси либо URL сайта.
2. Бот проверяет введенный IP-адрес прокси либо URL сайта с 10 разных стран на доступность.
3. Через несколько секунд бот выводит результат проверки с указанием отклика ресурса. Для удобства первые две страны в выводе: Россия и Украина.
...
├── code_style.py                    # Обработка стиля кода (в разработке)
...
├── middlewares.py                   # Промежуточные слои (защита от спама сообщений)
├── ping_utils.py                    # Логика проверки прокси адреса через пинг
├── states.py                        # Состояния пользователя
├── unit_tests_db_capitals_utils.py  # Тесты функций db_capitals_utils
├── unit_tests_download_database.py  # Тесты функций download_database
├── unit_tests_filter_utils.py       # Тесты функций filter_utils
├── unit_tests_geoip_utils.py        # Тесты функйий geoip_utils
